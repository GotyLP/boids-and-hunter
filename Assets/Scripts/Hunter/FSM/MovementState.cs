...
public class MovementState : IState
{
    ...
    public void OnUpdate()
    {
        ...
        var boids = GameManager.Instance.totalBoids;
        foreach (var item in boids)
        {
            if (Vector3.Distance(item.transform.position, _transform.position) < _radiusPersuit)
            {
                _fsm.ChangeState(HunterStatesNames.Hunting);
            }
        }
    }
